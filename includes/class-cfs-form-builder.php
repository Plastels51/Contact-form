...
class CFS_Form_Builder {
	...
	private function render_select_field( string $form_id, string $field, array $atts ): string {
		...
		$options     = $this->parse_options_string( $options_raw );
	 ...
	 */
	private function parse_options_string( string $raw ): array {
		if ( '' === trim( $raw ) ) {
			return array();
		}
	...
	private function render_multicheck_field( string $form_id, string $field, array $atts ): string {
		...
		$options_parsed = $this->parse_options_string( $options_raw );
		$options        = array();
		foreach ( $options_parsed as $val => $label_text ) {
			$options[ sanitize_key( $val ) ] = $label_text;
		}
